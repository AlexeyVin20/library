...
const FilterOperatorDropdown = ({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="bg-muted hover:bg-muted/50 px-1.5 py-1 text-gray-800 hover:text-gray-900 transition shrink-0">
        {getFilterOperatorTranslation(operator)}
      </DropdownMenuTrigger>
...
const FilterValueCombobox = ({
  ...
  text-gray-800 hover:text-gray-900 shrink-0"
      >
        <div className="flex gap-1.5 items-center">
          ...
          {filterValues?.length === 1
            ? (filterType === FilterType.STATUS ? getStatusTranslation(filterValues[0]) : filterValues[0])
            : `Выбрано: ${filterValues?.length}`}
        </div>
      </PopoverTrigger>
              ...
              <CommandGroup>
                {filterValues.map((value) => (
                  ...
                  >
                    ...
                    <FilterIcon type={value as FilterType} />
                    {filterType === FilterType.STATUS ? getStatusTranslation(value) : value}
                  </CommandItem>
                ))}
              </CommandGroup>
                    ...
                    {nonSelectedFilterValues.map((filter: FilterOption) => (
                      ...
                      >
                        ...
                        <span className="text-accent-foreground">
                          {filterType === FilterType.STATUS ? getStatusTranslation(filter.name as string) : filter.name}
                        </span>
                        {filter.label && (
...
// Функция для перевода типов фильтров и операторов на русский
const getFilterTypeTranslation = (type: FilterType): string => {
  switch (type) {
    case FilterType.STATUS:
      return "Статус";
    case FilterType.ASSIGNEE:
...
const getFilterOperatorTranslation = (operator: FilterOperator): string => {
  switch (operator) {
      ...
      {filters
        ...
        .map((filter) => (
          <div key={filter.id} className="flex gap-[1px] items-center text-xs">
            <div className="flex gap-1.5 shrink-0 rounded-l bg-muted px-1.5 py-1 items-center text-gray-800">
              <FilterIcon type={filter.type} />
              {getFilterTypeTranslation(filter.type)}
            </div>
            ...
            <Button
              ...
              className="bg-muted rounded-l-none rounded-r-sm h-6 w-6 text-gray-600 hover:text-gray-800 hover:bg-muted/50 transition shrink-0"
            >
              <X className="size-3" />
            </Button>
          </div>

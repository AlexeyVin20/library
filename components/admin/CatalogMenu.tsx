...
export default function CatalogMenu() {
  ...
  const categoryIcons = {
    ...
    "Professional": <Bookmark className="h-4 w-4 mr-2" />,
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div
          className={cn(
            "relative px-8 py-4 rounded-md text-2xl font-medium transition-all duration-300 hover:bg-gray-100 dark:hover:bg-gray-700 group min-w-[200px] text-center flex items-center justify-center gap-3 cursor-pointer",
      ...
      <DropdownMenuContent 
        className="w-96 max-h-[80vh] overflow-auto p-4 rounded-xl backdrop-blur-md bg-white/90 dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700 shadow-xl"
        align="center"
        sideOffset={12}
      >
        {loading ? (
          <div className="p-6 text-center">
            ...
            <p className="text-base text-gray-500">Загрузка данных...</p>
          </div>
        ) : (
          <>
            {/* Секция книг */}
            <DropdownMenuGroup>
              ...
              <DropdownMenuSeparator className="bg-primary/20 h-[2px]" />
              
              <div className="grid grid-cols-1 gap-2 my-2">
                <DropdownMenuItem asChild>
                  <Link href="/admin/books" className="flex items-center cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
                    <LayoutGrid className="mr-3 h-5 w-5 text-primary/70" />
                    <span className="font-medium">Все книги</span>
                  </Link>
                </DropdownMenuItem>
                
                <DropdownMenuItem asChild>
                  <Link href="/admin/books/create" className="flex items-center cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
              ...
              <DropdownMenuSub>
                <DropdownMenuSubTrigger className="flex items-center hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
                  <Bookmark className="mr-3 h-5 w-5 text-primary/70" />
                  <span>Популярные книги</span>
                </DropdownMenuSubTrigger>
                
                <DropdownMenuPortal>
                  <DropdownMenuSubContent 
                    className="max-h-[60vh] overflow-auto w-72 p-3 rounded-xl backdrop-blur-md bg-white/90 dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700 shadow-xl"
                  >
                    {books.slice(0, 8).map((book) => (
                      <DropdownMenuItem key={book.id} asChild>
                        <Link href={`/admin/books/${book.id}`} className="flex items-center gap-3 cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors">
                          <Book className="flex-shrink-0 h-5 w-5 text-primary/70" />
                    ...
                    {books.length > 8 && (
                      <DropdownMenuItem asChild>
                        <Link href="/admin/books" className="flex items-center justify-center text-primary mt-2 text-base">
            ...
            <DropdownMenuGroup>
              ...
              <div className="grid grid-cols-1 gap-2 my-2">
                <DropdownMenuItem asChild>
                  <Link href="/admin/journals" className="flex items-center cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
                    <LayoutGrid className="mr-3 h-5 w-5 text-primary/70" />
                    <span className="font-medium">Все журналы</span>
                  </Link>
                </DropdownMenuItem>
                
                <DropdownMenuItem asChild>
                  <Link href="/admin/journals/create" className="flex items-center cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
                    <PlusCircle className="mr-3 h-5 w-5 text-green-500" />
                    <span className="font-medium">Добавить журнал</span>
                  </Link>
                </DropdownMenuItem>
              </div>
              ...
              {Object.entries(categorizedJournals).map(([category, journals]) => (
                <DropdownMenuSub key={category}>
                  <DropdownMenuSubTrigger className="flex items-center hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors text-base">
                    {categoryIcons[category] || <BookOpen className="mr-3 h-5 w-5 text-primary/70" />}
                      ...
                      {journals.slice(0, 8).map((journal) => (
                        <DropdownMenuItem key={journal.id} asChild>
                          <Link href={`/admin/journals/${journal.id}`} className="flex items-center gap-3 cursor-pointer hover:bg-primary/10 rounded-lg px-3 py-2.5 transition-colors">
                            <BookOpen className="flex-shrink-0 h-5 w-5 text-primary/70" />
                      ...
                      {journals.length > 8 && (
                        <DropdownMenuItem asChild>
                          <Link 
                            ...
                            className="flex items-center justify-center text-primary mt-2 text-base"
                          >

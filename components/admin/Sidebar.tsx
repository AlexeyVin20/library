...
const Sidebar = () => {
  ...
  return <aside className="w-64 h-screen bg-blue border border-white dark:bg-gray-800 shadow-lg flex flex-col justify-between p-6">
      <div>
        ...
        <nav className="flex flex-col gap-4">
          ...
          return <Link href={link.route} key={link.route}>
                <div className={cn("flex items-center gap-4 p-3 rounded-md cursor-pointer transition-colors hover:bg-gray-100 dark:hover:bg-gray-700", isSelected && "bg-primary-admin shadow-sm")}>
                  <div className="relative w-6 h-6">
                    <Image src={link.img} alt="icon" fill className={cn("object-contain transition-all", isSelected && "brightness-0 invert")} />
                  </div>
        ...
        <div className="mt-6 flex justify-center items-center text-5xl">
          <Toggle pressed={theme === "dark"} onPressedChange={pressed => setTheme(pressed ? "dark" : "light")}>
            {theme === "light" ? "🌙" : "☀️"}
          </Toggle>
        </div>
      </div>

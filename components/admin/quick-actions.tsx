...
const ActionCard = ({ href, label, icon, color }: ActionProps) => {
  return (
    <Link href={href} className="block">
      <motion.div
        className={cn(
          "rounded-xl p-4 backdrop-blur-md shadow-lg border border-white/10",
...
export function QuickActions({ actions, title = "Быстрые действия" }: QuickActionsProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/30 backdrop-blur-xl rounded-xl p-5 shadow-lg border border-emerald-400/20"
    >
        ...
        {!isCollapsed && (
          ...
          >
            {actions.map((action, index) => (
              <ActionCard key={index} href={action.href} label={action.label} color={action.color} icon={action.icon} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <TooltipProvider delayDuration={300}>
            {actions.map((action, index) => (
              <Tooltip key={index}>
                  ...
                  <Link href={action.href}>
                    <motion.div
                      className={cn(
                        ...
                        "shadow-md border border-white/10 backdrop-blur-md",
                ...
                <TooltipContent
                  side="bottom"
                  className="bg-slate-900/90 backdrop-blur-md text-white border-emerald-500/30"
                >
                  {action.label}
                </TooltipContent>
              </Tooltip>

...
const ShelfItemContextMenu = ({
  ...
  return <div className="fixed z-50 w-64 rounded-lg shadow-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700" style={{
    left: position.x,
    top: position.y,
    maxWidth: '300px'
  }}>
      ...
      <div className="p-3 space-y-2">
        <div className="mb-2">
          ...
          {!isJournal && <p className="text-xs text-gray-600 dark:text-gray-400">
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
          <Link href={detailsPath} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-blue-700 bg-blue-100 rounded-md hover:bg-blue-200 dark:text-blue-300 dark:bg-blue-900/30 dark:hover:bg-blue-800/50">
            <ExternalLink size={14} className="mr-1" />
            Подробнее
          </Link>
          
          <button onClick={onRemove} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-red-700 bg-red-100 rounded-md hover:bg-red-200 dark:text-red-300 dark:bg-red-900/30 dark:hover:bg-red-800/50">
            <Trash size={14} className="mr-1" />
            Убрать
          </button>
        </div>
        
        {!isJournal && <div className="grid grid-cols-1 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
            <Link href={`/books/${item.id}`} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-green-700 bg-green-100 rounded-md hover:bg-green-200 dark:text-green-300 dark:bg-green-900/30 dark:hover:bg-green-800/50">

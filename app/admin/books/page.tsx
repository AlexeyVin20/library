            ...
            <div className="flex-1 pl-4 flex flex-col justify-between">
              <div>
                <Link href={`/admin/books/${book.id}`}>
                  <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">{book.title}</h3>
                </Link>
                <p className="text-sm text-gray-600 dark:text-gray-300">{book.authors}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{book.genre || ""}</p>
              </div>
...
const ThreeDBookView = ({ books, onDelete }: ViewProps) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-6 p-6">
      {books.map((book, index) => (
        <FadeInView key={book.id} delay={0.05 * index}>
          <div className="group text-gray-700 dark:text-white">
            ...
            >
              <Link href={`/admin/books/${book.id}`}>
                <p className="font-semibold line-clamp-1 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                  {book.title}
                </p>
              </Link>
    ...
    <div className="overflow-x-auto p-6">
      <table className="min-w-full divide-y divide-gray-200/50 dark:divide-gray-700/50">
        <thead className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 rounded-t-lg">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider w-12"></th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">Название</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">Автор</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">Жанр</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">Действия</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200/50 dark:divide-gray-700/50">
          {books.map((book, index) => (
            ...
            >
              ...
              <td className="px-4 py-3">
                <Link href={`/admin/books/${book.id}`}>
                  <p className="text-gray-700 dark:text-gray-200 font-medium hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">{book.title}</p>
                </Link>
              </td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{book.authors || "—"}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{book.genre || "—"}</td>
...
const ViewModeMenu = ({ viewMode, setViewMode }: ViewModeMenuProps) => {
  return (
    <NavigationMenu>
      <NavigationMenuList>
        <NavigationMenuItem>
          <NavigationMenuTrigger className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 text-gray-700 dark:text-gray-200 border border-white/20 dark:border-gray-700/30">
          ...
          <NavigationMenuContent className="backdrop-blur-xl bg-green/30 dark:bg-gray-800/20 p-2 rounded-xl border border-white/20 dark:border-gray-700/30 shadow-lg">
            <div className="grid gap-2 p-1 min-w-40">
              <motion.button 
                ...
                className="flex items-center gap-2 p-2 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-white/20 dark:hover:bg-gray-700/20 transition-colors"
                whileHover={{ x: 3 }}
              >
          ...
          >
            <div className="flex flex-wrap gap-4 items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Управление книгами</h1>
              </div>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <motion.button
                  ...
                  className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 text-gray-700 dark:text-gray-200 font-medium rounded-lg px-4 py-2 flex items-center gap-2 shadow-md border border-white/20 dark:border-gray-700/30"
                  whileHover={{ y: -3, boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.05)" }}
                  whileTap={{ scale: 0.98 }}
                >

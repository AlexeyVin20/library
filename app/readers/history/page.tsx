...
const ReservationItem: React.FC<{
  ...
  return <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-md shadow-lg rounded-lg p-4 md:p-6 mb-4 border border-white/20 dark:border-gray-700/30 hover:shadow-xl transition-shadow duration-300">
      <div className="flex flex-col md:flex-row gap-4">
        {reservation.book?.cover && <div className="flex-shrink-0 w-24 h-32 md:w-28 md:h-40 relative">
            <img src={reservation.book.cover} alt={reservation.book.title || "Обложка книги"} className="object-cover rounded-md w-full h-full" />
          </div>}
        <div className="flex-grow">
          <h3 className="text-lg md:text-xl font-semibold text-gray-800 dark:text-white mb-1 cursor-pointer hover:text-emerald-600 dark:hover:text-emerald-400" onClick={() => router.push(`/readers/books/${reservation.bookId}`)}>
            {reservation.book?.title || "Название неизвестно"}
          </h3>
          {reservation.book?.authors && <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          ...
          <div className="text-xs md:text-sm text-gray-700 dark:text-gray-300 space-y-1">
            ...
            {reservation.notes && <p className="flex items-start gap-1.5">
...
export default function ReservationHistoryPage() {
  ...
  return <div className="min-h-screen relative py-8 px-4">
      ...
      <div className="container mx-auto relative z-10">
        <Button variant="ghost" className="mb-6 flex items-center gap-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-gray-700 dark:text-gray-200" onClick={() => router.back()}>
        ...
        <div className="backdrop-blur-xl bg-white/30 dark:bg-gray-800/30 rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 md:p-8">
          ...
          {!loading && error && <div className="text-center py-10">

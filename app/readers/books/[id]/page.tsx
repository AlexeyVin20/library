...
export default function BookDetailsPage() {
  ...
  const nonActiveReservationStatuses = [
    "отменена", 
    "истекла", 
    "возвращена", 
  ...
  useEffect(() => {
    const fetchBookData = async () => {
      ...
      try {
        setLoading(true)
        const [bookResponse, favoritesResponse, allUserReservationsResponse] = await Promise.all([
          fetch(`${baseUrl}/api/books/${id}`),
          currentUserId ? fetch(`${baseUrl}/api/FavoriteBook/user/${currentUserId}`) : Promise.resolve(null),
          currentUserId ? fetch(`${baseUrl}/api/Reservation/user/${currentUserId}`) : Promise.resolve(null)
        ]);
        ...
        const coverUrl =
          bookData.cover || bookData.coverImage || bookData.coverImageUrl || bookData.image || bookData.coverUrl || bookData.imageUrl || ""
        ...
        if (favoritesResponse) {
          if (favoritesResponse.ok) {
            const favoriteBooksData: { bookId: string }[] = await favoritesResponse.json()
        ...
        if (currentUserId && allUserReservationsResponse) {
            if (allUserReservationsResponse.ok) {
              ...
              if (Array.isArray(allUserReservations)) {
                ...
                activeReservationForThisBook = allUserReservations.find(
                  res => res.bookId === id && 
                         res.status && // Убедимся что статус есть
  ...
  const handleToggleFavorite = async () => {
    if (!currentUserId) {
      toast({
        ...
        variant: "default",
        action: <Link href="/auth/login"><Button variant="outline" className="text-gray-800 dark:text-white border-gray-800 dark:border-white">Войти</Button></Link>,
      })
      return
    }
  ...
  const openReservationModal = () => {
    ...
    if (!book) return;
    if ((book.availableCopies ?? 0) <= 0 && !isBookEffectivelyReservedByCurrentUser) { // Проверяем доступность, только если еще не забронировано активно
       toast({
    ...
    if (isBookEffectivelyReservedByCurrentUser) { // Используем новое состояние
       toast({
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 rounded-2xl p-12 shadow-lg border border-white/20 dark:border-gray-700/30 text-center">
          ...
          <Button
            ...
            onClick={() => router.push("/readers/books")}
          >
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        <Button
          variant="ghost"
          className="mb-6 flex items-center gap-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
          onClick={() => router.back()}
        >
              ...
              <div className="flex flex-col items-center">
                <div className="relative w-full max-w-[300px] h-[400px] mx-auto">
                  {book.cover ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-emerald-100 dark:bg-emerald-900 rounded-xl">
                ...
                <div className="flex gap-2 mt-6 w-full max-w-[300px]">
                  <Button 
                    className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white disabled:bg-gray-400 disabled:cursor-not-allowed"
                    onClick={openReservationModal}
                    disabled={isBookEffectivelyReservedByCurrentUser || (book.availableCopies ?? 0) <= 0}
                  >
                    {isBookEffectivelyReservedByCurrentUser 
                      ? "Активно забронировано"
                  ...
                  <Button
                    ...
                    className={`border-gray-300 dark:border-gray-600 hover:border-red-500 dark:hover:border-red-400 ${
                      favoriteBookIds.has(book.id) ? "text-red-500 border-red-500 dark:text-red-400 dark:border-red-400" : "text-gray-700 dark:text-gray-300"
                    }`}
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input 
                ...
                className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500"
              />
            </div>

...
const FeaturedBook = ({
  ...
  return <motion.div className="relative overflow-hidden rounded-2xl shadow-lg h-[400px] md:h-[500px] backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 border border-white/20 dark:border-gray-700/30" whileHover={{
    scale: 1.02
  }} transition={{
    type: "spring",
    stiffness: 300,
    damping: 15
      ...
      {book.cover ? <NextImage src={book.cover || "/placeholder.svg"} alt={book.title} fill className="object-cover" priority /> : <div className="w-full h-full flex items-center justify-center bg-emerald-100 dark:bg-emerald-900">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-20 text-white">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(book)} className="text-white hover:text-emerald-400" aria-label="Toggle Favorite">
...
const FeaturedBookSmall = ({
  ...
  return <motion.div className="relative overflow-hidden rounded-xl shadow-lg h-[150px] backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 border border-white/20 dark:border-gray-700/30 mb-4" whileHover={{
    scale: 1.05
  }} transition={{
    type: "spring",
    stiffness: 300,
    damping: 15
      ...
      <div className="absolute top-0 left-0 bottom-0 p-4 z-20 text-white flex flex-col justify-center max-w-[70%]">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(book)} className="text-white hover:text-emerald-400 p-1" aria-label="Toggle Favorite">
...
const FeaturedBooksCarousel = ({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="flex flex-col justify-between">
        {books.slice(1, 4).map((book, index) => <FeaturedBookSmall key={book.id} book={book} isFavorite={favoriteBookIds.has(book.id)} onToggleFavorite={onToggleFavorite} />)}
      </div>
    </div>;
};
  ...
  return <div className="mb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {books.map((book, index) => <div key={book.id} className="relative">
            <Link href={`/readers/books/${book.id}`}>
              <motion.div className="group text-gray-700 dark:text-white" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
              ...
              height: "240px"
                ...
                scale: 1.05
                    ...
                    height: "260px"
                        ...
                        <BookCover coverColor="rgba(168, 212, 218, 0.5)" coverImage={book.cover || ""} variant="medium" className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 border border-white/20 dark:border-gray-700/30 rounded-xl shadow-lg w-full h-full" />
                      </div>
                    </div>
                  </motion.div>
                </div>
            ...
            <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(book)} className="absolute top-2 right-2 z-10 text-gray-700 dark:text-white hover:text-emerald-500 dark:hover:text-emerald-400 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-full p-1.5" aria-label="Toggle Favorite">
...
export default function ReadersHomePage() {
  ...
  if (loading) {
    return <div className="container mx-auto px-4 py-8 md:px-6 md:py-12 bg-gradient-to-br from-emerald-50 via-slate-50 to-teal-50 dark:from-slate-900 dark:via-gray-900 dark:to-teal-900 min-h-screen">
        {/* Header Section */}
        <FadeInView>
        ...
        <FadeInView delay={0.1}>
          <form onSubmit={handleSearch} className="mb-12 max-w-2xl mx-auto">
            <div className="relative backdrop-blur-xl bg-white/30 dark:bg-gray-800/30 border border-white/30 dark:border-gray-700/40 rounded-xl shadow-lg">
              <Input type="search" placeholder="Поиск книг, авторов, жанров..." className="w-full pl-12 pr-4 py-6 text-lg border-none focus:ring-2 focus:ring-emerald-500 rounded-xl bg-transparent placeholder:text-gray-500 dark:placeholder:text-gray-400" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
  ...
  return <div className="container mx-auto px-4 py-8 md:px-6 md:py-12 min-h-screen">
      ...
      {/* Featured Books Carousel */}
      {featuredBooks.length > 0 && <FadeInView delay={0.2}>
      ...
      {popularBooks.length > 0 && <FadeInView delay={0.3}>
          <BookCollection title="Популярные книги" icon={<TrendingUp className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />} books={popularBooks} viewAll="/readers/books?sort=popular" favoriteBookIds={favoriteBookIds} onToggleFavorite={handleToggleFavorite} />
        </FadeInView>}

      {/* New Books */}
      {newBooks.length > 0 && <FadeInView delay={0.4}>
          <BookCollection title="Новинки" icon={<Clock className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />} books={newBooks} viewAll="/readers/books?sort=newest" favoriteBookIds={favoriteBookIds} onToggleFavorite={handleToggleFavorite} />
        </FadeInView>}

      {/* Recommended Books */}
      {recommendedBooks.length > 0 && <FadeInView delay={0.5}>
          <BookCollection title="Рекомендации для вас" icon={<BookOpenCheck className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />} books={recommendedBooks} viewAll="/readers/books?category=recommended" favoriteBookIds={favoriteBookIds} onToggleFavorite={handleToggleFavorite} />
        </FadeInView>}
    </div>;
}